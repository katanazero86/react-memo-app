        ...
        <section className={`${classes.aboutWrap} pa-5`}>
            <div className={`${classes.about}`}>
                ...
                <p className={`${classes.projectInfo}`}>
                    해당 프로젝트는 React.js 를 학습 하고자 시작한 간단한 memo app 입니다.<br/>
                    ...
                    이번 프로젝트를 진행하면서 좋았던 점은 Atomic state management Jotai 를 가볍게(?) 사용했던 점이었습니다. Recoil 처럼 매우 간단하게 설정을 끝낼수
                    있었습니다. <br/>
                    Redux 도 훌륭한 상태 관리 라이브러리이지만 Jotai 는 이보다 훨씬 가볍고 미니멀하게 상태를 관리하게 도와준다고 생각합니다. <br/>
                    마지막으로, 이전에는 React 컴포넌트를 동적으로 가져오기 위해서 loadable components 모듈을 사용했어야 했습니다.(code splitting 을 위해 React
                    에서도 권장)<br/>
                    React 16.6.0 에서 출시한 React.lazy 를 사용해볼수 있어서 좋았으며, 아직은 SSR 및 라이브러리 코드 분할, Dynamic full import 기능이 지원되지
                    않지만 언젠가는 이러한 기능들도 지원 될거라 생각합니다.
                </p>
            </div>
        </section>
    )
