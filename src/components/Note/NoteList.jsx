...
const NoteItem = ({ targetItem, handleDeleteClick, handleDragStart, handleDragEnd, handleDrop, handleDragOver }) => {
  ...
  return (
    <div className={`${classes.noteListItem} pa-4`} ref={dragRef} data-id={targetItem.id}>
      ...
      <div onMouseDown={(e) => e.preventDefault()}>
        ...
        <p className={classes.memo}>{targetItem.memo}</p>
      </div>
    </div>
  );
};
  ...
  return (
    <React.Fragment>
      ...
      {confirm.isOpenConfirm ? (
        <ConfirmModal msg={confirm.msg} handleConfirmFunc={confirm.confirmFunc} handleCancelClick={handleCancelClick} />
      ) : (
        ''
      )}
    </React.Fragment>
